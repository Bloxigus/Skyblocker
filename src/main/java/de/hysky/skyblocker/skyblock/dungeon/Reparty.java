...
public class Reparty extends ChatPatternListener {
	...
	public Reparty() {
		...
		var handler = HypixelModAPI.getInstance().createHandler(ClientboundPartyInfoPacket.class, this::onPacket);
		handler.onError(this::onModAPIError);
	...
	private void onModAPIError(ErrorReason errorReason) {
		if (repartying) {
			CLIENT.player.sendMessage(Constants.PREFIX.get().append(Text.translatable("skyblocker.reparty.error")), false);
			LOGGER.error("[Skyblocker Reparty] The party info packet returned an unexpected error! {}", errorReason);

			this.repartying = false;
		}
	}

	private void onPacket(ClientboundPartyInfoPacket packet) {
		if (this.repartying) {
			boolean inParty = packet.isInParty();
			Map<UUID, ClientboundPartyInfoPacket.PartyMember> memberMap = packet.getMemberMap();

			UUID ourUuid = Objects.requireNonNull(CLIENT.getSession().getUuidOrNull());

			if (inParty && memberMap.get(ourUuid).getRole() == ClientboundPartyInfoPacket.PartyRole.LEADER) {
				sendCommand("/p disband", 1);
				int count = 0;

				for (Map.Entry<UUID, ClientboundPartyInfoPacket.PartyMember> entry : memberMap.entrySet()) {
					UUID uuid = entry.getKey();
					ClientboundPartyInfoPacket.PartyRole role = entry.getValue().getRole();

					//Don't invite ourself
					if (role != ClientboundPartyInfoPacket.PartyRole.LEADER) sendCommand("/p " + uuid.toString(), ++count + 2);
				}
